...
class ContiguousBufferReassembler {
   ...
   * @throws Error if the message is already complete.
   */
  public addBatched(chunks: Chunk[] | null, totalByteLength: number): Chunk | null {
    ...
    for (const currentChunk of chunks || []) {
      ...
      this.complete = currentChunk.endOfMessage;
      this.array?.set(currentChunk.payload, this.offset);
      this.offset += currentChunk.payload.byteLength;
      chunk = currentChunk;
    }
  ...
  private maybeResize(requiredLength: number): void {
    ...
    if (this.remaining < requiredLength) {
      const previousArray = this.array || new Uint8Array(0);
  ...
  public getMessage(): Uint8Array {
    ...
    const message = this.array?.subarray(0, this.offset) || new Uint8Array(0);
    this.complete = false;
    this.offset = 0;
    this.remaining = this.buffer?.byteLength || 0;
    return message;
  }
}
...
class UnreliableUnorderedReassembler {
  ...
  private moveQueuedChunks(): void {
    ...
    this.nextOrderedSerial = (chunk?.serial || 0) + 1;
 ...
 */
export class ReliableOrderedUnchunker extends AbstractUnchunker implements ReliableOrderedUnchunker {
 ...
 */
export class UnreliableUnorderedUnchunker extends AbstractUnchunker implements UnreliableUnorderedUnchunker {
  ...
  public add(chunkArray: Uint8Array): void {
    ...
    let reassembler: UnreliableUnorderedReassembler | undefined = this.reassemblers.get(chunk.id);
